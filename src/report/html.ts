 ...
 * claim at the exact moment someone is checking it.
 *
 * A guardrail test scans the emitted document for fetch-triggering constructs
 * and enumerates them one by one - `src=`, `<script`, `<link`, `url(`,
 * `@import`, `<iframe`, `<object`, `<embed`, `srcset=`, a meta refresh - plus
 * the rule that every `http(s)` URL sits inside an `<a href>`. It is a static
 * tripwire on a fixed template, not a load check: nothing opens the file, so
 * the claim holds only as far as the enumeration does, and only while this
 * renderer stays hand-written string concatenation (ADR-0045).
 */
...
export function renderHtml(report: RedactedReport): string {
  ...
  const body = [
    ...
    report.findings.length === 0
      ? '<p class="empty">No findings. Every check the active profile asked for ran ' +
        'and reported nothing. Portcall probes only the hosts that profile names, on ' +
        'port 443 only, so this is a clean result for that profile - not a clean bill ' +
        'of health for the network.</p>'
